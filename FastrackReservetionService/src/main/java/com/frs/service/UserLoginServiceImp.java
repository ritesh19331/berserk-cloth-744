...
public class UserLoginServiceImp implements UserLoginService {
	...
	public CurrentUserSession logInUser(UserLoginDTO dto) throws LogException {
		...
		if (currentUser.isPresent() || currentUserByMobile.isPresent()) {

			throw new LogException("User already Logged-In");
		}
		if (existingUserByEmail.getPassword().equals(dto.getPassword())) {
		...
		else if(existingUserByMobile.getPassword().equals(dto.getPassword())) {
			String key = RandomString.make(6);
			...
			throw new LogException("Password is wrong "+dto.getPassword()+". Enter right Password");
	}
